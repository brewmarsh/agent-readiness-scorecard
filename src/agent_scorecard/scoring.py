...
def score_file(filepath: str, profile: Dict[str, Any]) -> Tuple[int, str]:
    ...
    # Penalize specific functions that are too hard for an agent to read.
    ...
    for func in func_stats:
        if func['acl'] > 15:
            penalty = 5
            acl_penalty += penalty
            # Use "Hallucination Risk" to match Beta tests
            details.append(f"Hallucination Risk (ACL {func['acl']:.1f} > 15) in {func['name']} (-{penalty})")

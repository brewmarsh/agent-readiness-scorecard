...
class JavascriptAnalyzer(BaseAnalyzer):
    ...
    def score_file(
        ...
        # RESOLUTION: Modular penalty orchestration from cleanup branch
    ...
    # --- Private Penalty Helpers (Modular Logic) ---
    ...
    def _visit_complexity(self, n: Node) -> float:
        ...
        if n.type in ("if_statement", "for_statement", "for_in_statement", "while_statement", "do_statement", "switch_case", "catch_clause", "conditional_expression"):
            ...
            if operator and operator.text is not None and operator.text.decode("utf-8") in ("&&", "||"):
                count += 1.0
    ...
    def _visit_depth(self, n: Node, current_depth: int) -> int:
        ...
        if n.type in ("if_statement", "for_statement", "for_in_statement", "while_statement", "do_statement", "try_statement", "catch_clause", "switch_statement", "arrow_function", "function_expression"):
        ...
        for child in n.children:
            if child.type not in ("function_declaration", "method_definition"):

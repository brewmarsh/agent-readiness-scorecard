def generate_markdown_report(stats, final_score, path, profile, project_issues=None):
    ...
    for f_res in problematic_files:
        ...
        if red_functions:
            ...
            file_issues.append(
                f"- **Critical ACL**: Functions {fn_names} have Red ACL (>20). Prompt: 'Refactor functions in `{file_path}` with high cognitive load to bring ACL below 10. Split complex logic and reduce function length.'"
            )

        if f_res["type_coverage"] < 90:
            file_issues.append(
                f"- **Type Safety**: Coverage is {f_res['type_coverage']:.0f}%. Prompt: 'Add explicit type signatures to all functions in `{file_path}` to meet the 90% Type Safety Index requirement.'"
            )
...
def generate_advisor_report(stats, dependency_stats, entropy_stats, cycles):
    ...
    if high_acl_files:
        ...
        report += (
            "✅ No Hallucination Zones detected. Code is within reasoning limits.\n"
        )
    report += "\n"

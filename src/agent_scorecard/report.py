...
def generate_markdown_report(results):
    ...
    # FIXED: Changed 'stats' to 'file_results' to match variable scope
    high_acl_files = [s for s in file_results if s.get('acl', 0) > 15]
    ...
    if high_acl_files:
        ...
        for s in high_acl_files:
             acl_section += f"| {s['file']} | {s.get('acl', 0):.1f} |\n"
    else:
    ...
    # MERGED RETURN: Includes ACL section, Table, and Footer
    return summary + targets + prompts + acl_section + docs + table + "\n---\n*Generated by Agent-Scorecard*"
...
def generate_recommendations_report(results):
    ...
    for res in results["file_results"]:
        # Check ACL if available, otherwise fallback to complexity
        val = res.get("acl", res["complexity"])
        if val > 20:

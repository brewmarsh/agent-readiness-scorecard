...
def get_changed_files(base_ref: str = "origin/main") -> List[str]:
    ...
    try:
        ...
        console.print(
            f"[yellow]Warning: Git diff failed ({e}). Falling back to all files.[/yellow]"
        )
        return []
    except Exception as e:
        console.print(
            f"[yellow]Warning: Could not determine changed files ({e}).[/yellow]"
        )
        return []
...
def fix(path: str, agent: str) -> None:
    ...
    # RESOLUTION: Robust merging of user thresholds into profile
    user_thresholds = cfg.get("thresholds")
    if user_thresholds:
        current_thresholds: Thresholds = profile.get("thresholds") or {}
        current_thresholds.update(user_thresholds)
        profile["thresholds"] = current_thresholds
...
def score(
    ...
    if results["missing_docs"]:
        if final_verbosity != "quiet":
            console.print(
                f"[bold red]Missing Critical Agent Docs: {', '.join(results['missing_docs'])}[/bold red]"
            )
...
def check_prompts(input_file: Optional[str], plain: bool) -> None:
    ...
    if plain:
        ...
        console.print(
            Panel(f"[bold cyan]Prompt Analysis: {name}[/bold cyan]", expand=False)
        )
        ...
        stats.append(
            {
                ...
                "type_coverage": res["type_coverage"],
                "function_metrics": res["function_metrics"],
                "acl": m_acl,

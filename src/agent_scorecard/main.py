...
def _print_project_issues(results: AnalysisResult, verbosity: str) -> None:
...
@click.option("--plain", is_flag=True, help="Output plain text results for CI.")
def check_prompts(path: str, plain: bool) -> None:
    ...
    if plain:
        ...
        for key, val in result["results"].items(): click.echo(f"{key}: {val}")
    else:
...
def score(path: str, agent: str, fix: bool, report_path: str, verbosity: str, badge: bool, diff_base: Optional[str]) -> None:
    ...
    thresholds = cast(Thresholds, cfg.get("thresholds"))

    if final_verbosity != "quiet":
        console.print(Panel("[bold cyan]Running Agent Scorecard[/bold cyan]", expand=False))

    if fix:
        apply_fixes(path, cast(Profile, copy.deepcopy(PROFILES.get(agent, PROFILES["generic"]))))
...
def advise(path: str, output_file: Optional[str]) -> None:
    ...
    cfg = load_config(path)
    results = analyzer.perform_analysis(path, "generic", thresholds=cast(Thresholds, cfg.get("thresholds")))

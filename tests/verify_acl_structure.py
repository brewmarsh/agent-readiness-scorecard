...
def verify():
    ...
    print(
        f"Flat/Long File ACL: {acl_a}"
    )  # Expected: (1*2) + (1*1.5) + (200/50) = 2 + 1.5 + 4 = 7.5
    print(
        f"Short/Nested File ACL: {acl_b}"
    )  # Expected: (5*2) + (2*1.5) + (20/50) = 10 + 3 + 0.4 = 13.4

    assert acl_a < acl_b
    print(
        "Verification Successful: Flat/Long files score better than Short/Nested files."
    )
